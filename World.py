...
class World:
	...
	def move(self, source, dest):
		...
		team = path[-1]
		while True:
			self.print_move(team.position, team.parent.position)
			team = team.parent
			if team.position.row == source.position.row and team.position.column == source.position.column:
				return None

	def print_move(self, start, end):
		# Nord
		if start.row == end.row + 1:
			print("S ",end='')
		# Sud
		elif start.row == end.row - 1:
			print("N ",end='')
		# West
		elif start.column == end.column - 1: 
			print("W ",end='')
		else:
			print("E ",end='')
	...
	def find_in_graph(self, team):
		...
		while True:
			...
			for n2 in open_l:
				...
				n2.g = n1.g + self.path_distance(path, n1)
				n2.parent = n1
	...
	def path_distance(self, p, source):
		dest = p[-1]
		size_path = 0
		while True:
			size_path += 1
			dest = dest.parent
			if dest.position.row == source.position.row and dest.position.column == source.position.column:
				# Ajouter la liste des états en retour pour deplacer l'equipe
				return size_path
